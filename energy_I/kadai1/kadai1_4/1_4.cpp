...
#include <string.h>
#include <math.h>
#define NX 40 /* NX:x方向格子数 */
#define NY 40 /* NY:y方向格子数 */
...
{
     int ix,iy;
     for (iy=(NY/4)+1;iy<=NY;iy++){/*U（右側面上3/4）*/
       UN[NX][iy] = 0.0;
     }
     for (iy=1;iy<=NY/4;iy++){/*U（右側面下1/4）∂U/∂x=0 */
       UN[NX][iy] = UN[NX-1][iy];
     }
     for (iy=1;iy<=(NY*3/4);iy++){/*U（左側面下3/4）*/
       UN[0][iy] = 0.0;
     }
     for (iy=(NY*3/4)+1;iy<=NY+1;iy++){/*U（左側面上1/4）u_in*/
       UN[0][iy] = u_in;
     }
     ...
     for (iy=(NY/4)+1;iy<=NY-1;iy++){/*V（右側面上3/4）*/
       VN[NX+1][iy] = -VN[NX][iy];
     }
     for (iy=1;iy<=NY/4;iy++){/*U（右側面下1/4）V=0 */
       VN[NX+1][iy] = 0;
     }
     for (iy=1;iy<=(NY*3/4);iy++){/*V（左側面下3/4）*/
       VN[0][iy] = -VN[1][iy];
     }
     for (iy=(NY*3/4)+1;iy<=NY-1;iy++){/*V（左側面上1/4）V=0 */
       VN[1][iy] = 0;
     }
...
{
     int ix,iy;
      for (iy=(NY/4)+1;iy<=NY+1;iy++){/*右側面上3/4 Θ=+0.5 */
        TN[NX+1][iy] = 2.0 * ( +0.5 ) - TN[NX][iy];
      }
      for (iy=0;iy<=NY/4;iy++){/*右側面下1/4  ∂Θ/∂x=0 */
        TN[NX+1][iy] = TN[NX][iy];
      }
      for (iy=0;iy<=(NY*3/4);iy++){/*左側面下3/4*/
        TN[0][iy] = 2.0 * ( +0.5 ) - TN[1][iy];
      }
      for (iy=(NY*3/4)+1;iy<=NY+1;iy++){/*左側面上1/4 低温空気Θl=-0.5*/
        TN[0][iy] = 2.0 * ( -0.5 ) - TN[1][iy];
      }
