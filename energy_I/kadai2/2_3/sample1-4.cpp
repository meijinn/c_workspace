  ...
  { /*新規計算の場合*/
    ...
    { /*Tの初期値設定(領域内は高温(+0.5)と低温(-0.5)の中間温度)*/
      ...
      {                   /*（注意）浮力項の計算で温度の配列を使用して*/
        TN[ix][iy] = 10.0; /*いるので等温場でもT=0として初期条件だけは設定する*/
    ...
    for (ix = 1; ix <= NX; ix++)
    { /*Tの境界：上面（断熱）*/
      if (ix >= NX*3/8 && ix <= NX*5/8)
      {
        TN[ix][NY+1] = 2.0 * 10 - TN[ix][NY];
      }else{
        TN[ix][NY + 1] = TN[ix][NY];
      }
    }
...
void velbnd()
{
  ...
  // for (iy = 1; iy <= NY*1/4; iy++){
  //     UN[(NX*1/4)+1][iy] = -UN[NX*1/4][iy];
  // }
  for (ix = 0; ix <= NX; ix++)
  { /*U（上面）*/
    if(ix >= NX*3/8 && ix <= NX*5/8)
  ...
  for (iy = 1; iy <= NY*1/4; iy++){
        UN[NX*1/4][iy] = 0;
        VN[NX*1/4][iy] = -VN[(NX*1/4)+1][iy];
  }
  //真ん中縦線
  for (iy = 1; iy <= NY * 1/4; iy++){
      UN[NX*2/4][iy] = 0;
      VN[NX*2/4][iy] = -VN[(NX*2/4)+1][iy];
  }
  //真ん中から右にかけて
  for (ix = NX*2/4; ix <= NX*3/4; ix++){
      UN[ix][NY*1/4] = -UN[ix][(NY*1/4)+1];
      VN[ix][NY*1/4] = 0;
  }
  //右から上
  for (iy = NY*1/4; iy <= NY*2/4 ; iy++){
      UN[NX*3/4][iy] = 0;
      VN[NX*3/4][iy] = -VN[(NX*3/4)+1][iy];
  }
  //右上から右
  for (ix = NX*3/4; ix <= NX; ix++){
      UN[ix][NY*2/4] = -UN[ix][(NY*2/4)+1];
      VN[ix][NY*2/4] = 0;
  }
}
/*温度の境界条件の処理*/
void tbnd()
{
  ...
  for (iy = 0; iy <= NY; iy++)
  { /*右側面*/
      TN[NX + 1][iy] = 2.0 * (+25) - TN[NX][iy];
  }
  //真ん中縦線
  for (iy = 1; iy <= NY * 1/4; iy++){
      TN[NX*2/4][iy] = TN[(NX*2/4)+1][iy];
  }
  //真ん中から右にかけて
  for (ix = NX*2/4; ix <= NX*3/4; ix++){
      TN[ix][NY*1/4] = TN[ix][(NY*1/4)+1];
  }
  //右から上
  for (iy = NY*1/4; iy <= NY*2/4 ; iy++){
      TN[NX*3/4][iy] = TN[(NX*3/4)+1][iy];
  }
  //右上から右
  for (ix = NX*3/4; ix <= NX; ix++){
      TN[ix][NY*2/4] = TN[ix][(NY*2/4)+1];
  }


  for (iy = 0; iy <= NY + 1; iy++)
  { /*左側面*/
      TN[0][iy] = 2.0 * (+25) - TN[1][iy];
  }
  //横
  for (ix = 1; ix <= NX*1/4; ix++){
      TN[ix][(NY*1/4)-1] = TN[ix][NY*1/4];
  }
  //縦
  for (iy = 1; iy < NY*1/4; iy++){
      TN[NX*1/4][iy] = TN[(NX*1/4)+1][iy];
  }
  ...
  { /*上面*/
    ...
    TN[ix][NY + 1] = 2*(+25)-TN[ix+1][NY+1];
    //TN[ix][NY + 1] = TN[ix][NY];
    }
  }
  for (ix = 1; ix <= NX; ix++)
  { /*下面*/
      TN[ix][0] = 2*(+25)-TN[ix+1][0];
  }
  for (ix = 1; ix <= NX; ix++){
      if (ix < NX*1/4){
        TN[ix][NY*1/4] = 2.0 * (+100) - TN[ix+1][NY*1/4]; 
    }
  }
}
void prout() /*データ出力用*/
